...
describe RemoteFactoryBot::ConfigApplier do

  describe '.post' do
    let(:json) {
      { :user => { :first_name => "Sam", :last_name => "Iam"}}
    }
    let(:hash_to_dot_klass)      { double('RemoteFactoryBot::HashToDot') }
    ...
    describe '.apply_config' do
      describe 'when configured to return root key' do
        it 'should return a hash with a root key when initialized with a hash that has a root key' do
          response = RemoteFactoryBot::ConfigApplier.apply_config(json, :with_root => true)
      ...
      describe 'when configured to not return root key' do
        ...
        let(:factory_bot_json_parser) { double('RemoteFactoryBot::FactoryBotJsonParser', without_root: { :first_name => 'Sam', :last_name => 'Iam'}) }

        it 'should not return a hash with a root key when initialized with a hash that has a root key' do
          response = RemoteFactoryBot::ConfigApplier.apply_config(json, :return_with_root => false,
                                                                         :factory_bot_json_parser => factory_bot_json_parser)
        ...
        it 'should not return a hash with a root key when initialized with a hash that does not have a root key' do
          ...
          response              = RemoteFactoryBot::ConfigApplier.apply_config(hash_without_root_key, :return_with_root => false,
                                                                                                       :factory_bot_json_parser => factory_bot_json_parser)
          expect(response[:first_name]).to eq('Sam')
        end
      end
      ...
      it 'should return an object that responds to dot notation' do
        ...
        response = RemoteFactoryBot::ConfigApplier.apply_config(json, :return_response_as  => :dot_notation,
      ...
      it 'should not return root hash key and should return an object that responds to dot notation' do
        factory_bot_json_parser = double('RemoteFactoryBot::FactoryBotJsonParser', without_root: { :first_name => 'Sam', :last_name => 'Iam'})
    ...
    describe 'when configured to return active resource objects' do
      it 'should return an active resource object' do
        json_to_active_resource_klass = double('RemoteFactoryBot::JsonToActiveResource')
        expect(json_to_active_resource_klass).to receive(:convert).with(json)
        RemoteFactoryBot::ConfigApplier.apply_config(json, :return_as_active_resource     => true,
